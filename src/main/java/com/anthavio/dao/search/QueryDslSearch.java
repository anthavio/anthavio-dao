package com.anthavio.dao.search;
...
import com.anthavio.dao.search.criteria.DateSearchType;
...
public class QueryDslSearch {
	...
	public <P> QueryDslSearch joinFetch(Path<? extends Collection<P>> o) {
		jpaQuery.join(o).fetch();
	...
	public <P> QueryDslSearch leftJoinFetch(Path<? extends Collection<P>> o) {
		jpaQuery.leftJoin(o).fetch();
		collectionFetch = true;
		return this;
	}
	...
	public <RT> PagedResult<RT> listPaged(Expression<RT> selectExpr, SimpleExpression<?> countExpr,
		...
		if (total > 0) {
			...
			for (JoinExpression join : fetchJoins) {
				join.getFlags().add(JPQLQueryMixin.FETCH);
			}
			List<JoinExpression> fetchAllJoins = fetchJoinGroups[1];
			for (JoinExpression join : fetchAllJoins) {
				join.getFlags().add(JPQLQueryMixin.FETCH_ALL_PROPERTIES);
			}
	...
	private List<JoinExpression>[] extractFetchJoins() {
		...
		for (JoinExpression join : joins) {
			...
			if (flags.contains(JPQLQueryMixin.FETCH)) {
				flags.remove(JPQLQueryMixin.FETCH);
				fetchJoins.add(join);
				//if (join.getTarget() instanceof Path<?>) {
				//}
			} else if (flags.contains(JPQLQueryMixin.FETCH_ALL_PROPERTIES)) {
				flags.remove(JPQLQueryMixin.FETCH_ALL_PROPERTIES);
	 ...
	 */
	public QueryDslSearch eq(DateTimeExpression<Date> property, Date date) {
		if (date != null) {
	...
	public QueryDslSearch range(DateTimeExpression<Date> property, DateCriteria criteria) {
