package net.anthavio.cdao.criteria;
...
public class Example extends Group {
		...
		try {
			...
			for (PropertyDescriptor pd : propertyDescriptors) {
				...
				if (excludeNulls && value != null) {
					log.debug("Including: " + name + " of class: " + className + " with value: " + value
		...
		if (!"boolean".equals(className)) {
			if (excludeZeroes) {
				...
				if (number.intValue() != 0) {
					...
					log.debug("Excluding primitive value: " + value + " for property named: " + name
							+ " of class: " + className);
				}
			} else {
				this.eq(name, value);
	...
	private void handleNonPrimitives(String name, Object value, String className, boolean isNull,
			boolean isString) {
