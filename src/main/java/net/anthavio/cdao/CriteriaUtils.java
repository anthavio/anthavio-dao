package net.anthavio.cdao;

import java.util.HashSet;
import java.util.Set;

import net.anthavio.cdao.criteria.Between;
import net.anthavio.cdao.criteria.Comparison;
import net.anthavio.cdao.criteria.Criterion;
import net.anthavio.cdao.criteria.Group;
import net.anthavio.cdao.criteria.Operator;
import net.anthavio.cdao.criteria.OrderBy;
import net.anthavio.cdao.criteria.Select;
import net.anthavio.cdao.criteria.VerifiedBetween;
import net.anthavio.cdao.join.EntityJoin;
import net.anthavio.cdao.join.Join;
import net.anthavio.cdao.join.JoinType;
import net.anthavio.cdao.join.RelationshipFetch;
import net.anthavio.cdao.join.RelationshipJoin;
import net.anthavio.cdao.join.SimpleRelationshipJoin;
...
public class CriteriaUtils {
	...
	public static void addComparisonToQueryString(Comparison comparison, StringBuilder builder,
			Set<String> names) {
	...
	public static String constructSelect(Select[] selects, String newSelectStatement,
			String entityName, String instanceName, boolean distinctFlag) {
	...
	public static String createQuery(Class<?> clazz, Select[] selects, String newSelectStatement,
			boolean distinctFlag, OrderBy[] orderBy, Join[] joins, final Group group) {
		StringBuilder sbQuery = new StringBuilder(255);
		final String sQuery = sbQuery.append(
				CriteriaUtils.constructSelect(selects, newSelectStatement, GenericDaoUtils
						.getEntityName(clazz), "o", distinctFlag)).append(
				CriteriaUtils.constructFrom(clazz, joins)).append(CriteriaUtils.constructJoins(joins))
				.append(CriteriaUtils.constuctWhereClause(group)).append(
						CriteriaUtils.constructOrderBy(orderBy)).toString();
	...
	public static String createCountQuery(final Group group, Class<?> type, boolean distinct) {
		final StringBuilder sbquery = new StringBuilder("SELECT count(" + (distinct ? "DISTINCT " : "")
				+ "o ) " + " FROM ");
		sbquery.append(GenericDaoUtils.getEntityName(type));
		sbquery.append(" ");
